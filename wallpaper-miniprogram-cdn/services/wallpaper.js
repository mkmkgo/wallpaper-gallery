...
function buildRawUrl(path) {
  if (!path) return "";
  return config.API_CONFIG.RAW_BASE + "/latest" + encodePath(path);
}
  ...
  return api.getHotStats().then(function(hotStats) {
    ...
    return api.getWallpapersByIds(topIds).then(function(wallpapers) {
      ...
      for (var j = 0; j < hotStats.length; j++) {
        var stat = hotStats[j];
        var matched = wpMap[stat.image_id];
        if (matched) {
          matched._hotScore = (stat.views || 0) + (stat.downloads || 0) * 2;
          scored.push(matched);
        }
      }
